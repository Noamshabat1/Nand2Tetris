...
class JackTokenizer:
    ...
    @staticmethod
    def is_empty_line(arr: list[list], i: int) -> bool:
        return arr[i] == ""
    ...
    @staticmethod
    def is_inline_comment(arr: list[list], i: int) -> bool:
        return arr[i][0:2] == "//"

    @staticmethod
    def not_a_string(arr):
    ...
    def drop_white_space_from_command(arr: list, i: int) -> str:
        ...
        if JackTokenizer.not_a_string(line_list):
            line = line_list[0]
    ...
    @staticmethod
    def remove_comments(input_lines: list) -> list:
        """
        ...
        while flag:
            if ("/*" in input_lines[cur]) and JackTokenizer.not_a_string(input_lines[cur].partition("/*")):
    ...
    def delete_white_spaces(self, input_arr: list) -> list:
        ...
        while counter < number_of_lines:
            if counter == number_of_lines:
                ...
                if not (self.is_inline_comment(input_arr,counter) and not self.is_empty_line(input_arr, counter)):
                    tmp = self.drop_white_space_from_command(input_arr, counter)
                    if tmp != "":
                        clean_lines.append(tmp)
            counter += 1
        return self.remove_comments(clean_lines)
